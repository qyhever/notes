消息“远程：用户名或令牌无效。Git 操作不支持密码验证。” 表示您正在尝试使用用户名和密码对 Git 操作（例如git push、git pull或git clone）进行身份验证，但远程 Git 托管服务（例如 GitHub、Bitbucket）不再支持通过 HTTPS 对 Git 操作进行基于密码的身份验证。 这些服务要求使用个人访问令牌 (PAT) 或 SSH 密钥。
